// Build the MWAA AI Council Word document.
...
const OUT = path.join(REPO, 'outputs/stage4/infrastructure-vs-intelligence.docx');

// ---------- inline formatting ----------
// Parse a line with **bold**, *italic*, `code` into an array of TextRun.
function parseInline(line) {
  const runs = [];
  // Tokenize by **, *, ` — keeping state simple.
  let i = 0;
  while (i < line.length) {
...
function bodyFromMarkdown(md) {
  ...
  // Skip the opening H1 and the draft metadata block (we have a cover page).
  // We resume at the first "## 1. Thesis".
  let start = 0;
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].startsWith('## 1. Thesis')) { start = i; break; }
  }
  ...
  while (i < lines.length) {
    ...
    // Horizontal rule — skip (visual separators not needed in docx)
    if (line.trim() === '---') { i++; continue; }

    // Blank line — skip; paragraphs already separated by structure
    if (line.trim() === '') { i++; continue; }
    ...
    // Default: a paragraph (single line in this source — each para is one line)
    children.push(new Paragraph({
      spacing: { after: 160 },
...
function coverPage() {
  ...
  out.push(new Paragraph({
    ...
    spacing: { after: 720 },
    children: [new TextRun({
      text: 'Pressure-testing the next billion dollars of airport CapEx',
      italics: true, size: 32, font: 'Arial'
    })],
  }));
  ...
  out.push(new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { after: 120 },
    children: [new TextRun({ text: 'MWAA AI Council', size: 28, font: 'Arial' })],
  }));

  out.push(new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { after: 720 },
    children: [new TextRun({ text: 'April 17, 2026', size: 24, font: 'Arial' })],
  }));
...
function methodologyAppendix() {
  ...
  out.push(new Paragraph({
    spacing: { after: 160 },
    children: [new TextRun({
      text: 'This document was produced by a multi-agent AI system operated by ________________ [name / role]. The system consisted of eight specialized agents running on Anthropic\u2019s Claude platform:',
    })],
  }));
...
const doc = new Document({
  ...
  title: 'Infrastructure vs. Operational Intelligence',
  description: 'Pressure-testing the next billion dollars of airport CapEx',
  styles: {
    default: {
    ...
    paragraphStyles: [
      {
        ...
        quickFormat: true,
        run: { size: 36, bold: true, font: 'Arial', color: '000000' }, // 18pt
      ...
      },
      {
        ...
        quickFormat: true,
        run: { size: 24, bold: true, italics: true, font: 'Arial', color: '000000' }, // 12pt
  ...
  sections: [
    // Section 1: cover page — no page number in footer
    {
      properties: {
        page: {
    ...
    // Section 2: TOC + body + appendix, with page numbers
    {
      properties: {
        page: {
