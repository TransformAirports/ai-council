...
const OUT = path.join(REPO, 'outputs/stage4/saarinen-iad-redesign.docx');
...
function coverPage() {
  ...
  out.push(new Paragraph({
    ...
    children: [new TextRun({
      ...
      italics: true, size: 26, font: 'Arial'
    })],
  }));
  ...
  out.push(new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { after: 720 },
    children: [new TextRun({ text: 'May 29, 2026', size: 24, font: 'Arial' })],
  }));
...
function methodologyAppendix() {
  ...
  out.push(new Paragraph({
    spacing: { after: 160 },
    children: [new TextRun({
      text: 'This document was produced by the Transform Airports AI Council, a multi-agent system operated by ________________ [name / role]. The Council seated thirteen specialized agents for this run, running on Anthropic’s Claude platform:',
    })],
  }));

  const bullets = [
    'Eight standard research agents — Infrastructure Economist, Operations Analyst, Technology Scout, Contrarian, Chief Engineer, Airline Commercial Strategist, Regulatory/Political Analyst, and Aviation Historian — that independently gathered evidence without reading each other’s work.',
    ...
    'One new agent created for this run — the Architectural Historian — that carried the primary documentary record of Saarinen’s design principles, the architectural lineage, and the anti-hagiography discipline that separates intent from aesthetic.',
    'A Strategist that synthesized the thirteen briefs into a single argumentative draft.',
...
const doc = new Document({
  ...
  title: 'If Saarinen Were Designing Dulles Today',
  description: 'A position paper on a faithful reinterpretation of Saarinen’s design philosophy for IAD in 2026',
  styles: {
    default: {
